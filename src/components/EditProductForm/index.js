...
class EditProductForm extends React.Component {
	...
	handleSubmit = e => {
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				this.props.handleSubmit({ ...values, imageUrl: this.state.imageUrl, _id: this.props._id });
			} else {
			}
		});
	};
	...
	render() {
		...
		const { productName, dddLink, description, defaultRetail, defaultWholeSale, defaultUnits } = this.props;
								...
								<Row gutter={20}>
									...
									<Col span={12}>
										<Form.Item style={{ display: "inline-block" }}>
											{getFieldDecorator("defaultUnits", { initialValue: defaultUnits })(
												<InputNumber min={2} />
											)}
										</Form.Item>
										<Text
...
export default Form.create({ name: "newproduct" })(EditProductForm);
